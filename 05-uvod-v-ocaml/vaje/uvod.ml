...
let rec starting_element seznam = match seznam with
...
let rec multiply seznam = match seznam with
       | [] -> 1
	| x :: xs -> x * multiply xs
...
let rec sum_int_pairs seznam = match seznam with
       | [] -> []
...
let rec get k seznam = match seznam with
...
let rec divide k seznam = match (k, seznam) with
	...
	| (k, x :: xs) -> 
		let (seznam1, seznam2) = divide (k - 1) xs in (x :: seznam1, seznam2)
...
let rec rotate  n seznam =
	let (seznam1, seznam2) = divide n seznam in seznam2 @ seznam1
...
let rec remove x seznam = match seznam with
	| [] -> []
	| y :: ys ->
...
let rec max_on_components seznam1 seznam2 = match (seznam1, seznam2) with
...
let rec second_largest seznam =
	let rec largest = function
		| [] -> failwith "Seznam nima nobenega kandidata!"
		| x :: [] -> x
